...
def lista_tupla(lista: list, actividad: int) -> Callable:
    ...
    def letra_nombres(letra_inicial: str):
        listaNombres = [
            name[1] for name in lista
            if name[1][1][0].lower() == letra_inicial.lower()
        ]
    ...
    def plataforma(plataform: str, ordenar: str):
        listaPlatf = [(name[1], name[2], name[3]) for name in lista
                      if name[2][1].lower() == plataform.lower()]
    ...
    def ventas(orden: str):
        listaProm = [(name[1], ("Avg_Sales", promedio(name[6:10])))
                     for name in lista]
    ...
    #--------------------5-----------------------#
    def generos(año: str):
        listaPlatf = [(name[0], name[1], name[4]) for name in lista
                      if name[3][1] == año]
