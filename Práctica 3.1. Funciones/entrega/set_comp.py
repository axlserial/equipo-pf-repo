...
def filter_columns(columns: list[str], values: list,
                   h: tuple[str, ...]) -> Callable:

    def inner(data: tuple) -> tuple:
        return tuple([
            d for d in data
            if all([d[h.index(c)] == v for c, v in zip(columns, values)])
        ])
...
def filter_column_range(column: str, value1, value2,
                        h: tuple[str, ...]) -> Callable:

    def inner(data: tuple) -> tuple:
        return tuple(
            [d for d in data if value1 <= d[h.index(column)] <= value2])
...
def filter_column_nums(column: str, value, h: tuple[str, ...]) -> Callable:
    ...
    def inner(data: tuple) -> tuple:
        return tuple(
            [d for d in data if float(d[h.index(column)]) > float(value)])

    return inner


# Closure para encontrar una subcadena en una columna de una tupla.
def find_substring(column: str, substring: str, h: tuple[str,
                                                         ...]) -> Callable:

    def inner(data: tuple) -> tuple:
        return tuple(
            [d for d in data if d[h.index(column)].find(substring) != -1])
...
def first_set_comp(data: tuple[tuple[str, ...], ...],
                   h: tuple[str, ...]) -> set[str]:
...
def second_set_comp(data: tuple[tuple[str, ...], ...],
                    h: tuple[str, ...]) -> set[tuple[str, str]]:
...
def third_set_comp(data: tuple[tuple[str, ...], ...],
                   h: tuple[str, ...]) -> set[tuple[str, str]]:
    get_name = u.select_column("Name", h)
...
def fourth_set_comp(data: tuple[tuple[str, ...], ...],
                    h: tuple[str, ...]) -> set[tuple[str, str]]:
    get_name = u.select_column("Name", h)
    get_global_sales = u.select_column("Global_Sales", h)
    ...
    games = {(get_name(game), get_global_sales(game))
             for game in get_substring_Sonic(get_filters(data))}
...
def fifth_set_comp(data: tuple[tuple[str, ...], ...],
                   h: tuple[str, ...]) -> set[str]:
    ...
    games = {
        get_publisher(game)
        for game in get_filter_2013_2016(get_filter_Platform(data))
    }
...
def conjunto_p(data: tuple[tuple[str, ...], ...],
               h: tuple[str, ...]) -> set[tuple[str, str]]:
    get_name = u.select_column("Name", h)
    get_global_sales = u.select_column("Global_Sales", h)
    get_filters = filter_columns(["Publisher", "Platform"], ["Sega", "3DS"], h)
    games = {(get_name(game), get_global_sales(game))
             for game in get_filters(data)}
    return games


# Conjunto de los nombres de las plataformas que tengan juegos del genero Action
def conjunto_q(data: tuple[tuple[str, ...], ...], h: tuple[str,
                                                           ...]) -> set[str]:
    get_platform = u.select_column("Platform", h)
...
def conjunto_u(data: tuple[tuple[str, ...], ...],
               h: tuple[str, ...]) -> set[tuple[str, str]]:
    get_name = u.select_column("Name", h)
    get_na_sales = u.select_column("NA_Sales", h)
